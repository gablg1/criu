						...
						unsigned int mnt_id)
{
	...
	for (m = list; m != NULL; m = m->next) {
		...
		if (m->fstype->code == FSTYPE__OVERLAYFS) {
			...
			if (ret_stat == 0 && s_dev == f_stat.st_dev && st_ino == f_stat.st_ino)
				ret = m;
		}
	}
					...
					unsigned int st_ino, unsigned int mnt_id)
{
	...
	mi = __lookup_overlayfs(mntinfo, rpath, s_dev, st_ino, mnt_id);
